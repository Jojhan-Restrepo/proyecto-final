    ...
    , clic(true)
{
    ...
    QPushButton *timeButton = new QPushButton("Modificar Tiempo"); // Nuevo botón
    ...
    QGraphicsProxyWidget *timeButtonItem = menuScene->addWidget(timeButton); // Añadir el botón a la escena

    startButtonItem->setPos(250, 100);
    scoresButtonItem->setPos(250, 200);
    timeButtonItem->setPos(250, 300); // Posición del nuevo botón

    connect(startButton, &QPushButton::clicked, this, &MainWindow::startGame);
    connect(scoresButton, &QPushButton::clicked, this, &MainWindow::showScores);
    connect(timeButton, &QPushButton::clicked, this, &MainWindow::modifyTime); // Conectar el nuevo botón a la función
...
void MainWindow::modifyTime() {
    bool ok;
    double newTimeInterval = QInputDialog::getDouble(this, tr("Modificar Tiempo de Fenómenos Físicos"),
                                                     tr("Ingrese el nuevo intervalo de tiempo en milisegundos:"),
                                                     60.0, 1.0, 10000.0, 1, &ok);
    if (ok) {
        // Cambiar el intervalo del temporizador principal del juego
        timer->setInterval(static_cast<int>(newTimeInterval));

        // Cambiar el intervalo del temporizador de disparos de enemigos, si existe
        QList<QTimer *> allTimers = this->findChildren<QTimer *>();
        for (QTimer *t : allTimers) {
            if (t != timer) { // Evitar cambiar el temporizador principal de juego
                t->setInterval(static_cast<int>(newTimeInterval));
            }
        }

        QMessageBox::information(this, tr("Tiempo Modificado"),
                                 tr("El tiempo de los fenómenos físicos se ha cambiado a %1 ms").arg(newTimeInterval));
    }
}
